        ...
        <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 prose prose-stone lg:prose-xl prose-img:rounded-3xl prose-img:shadow-xl prose-headings:font-serif prose-headings:font-bold prose-headings:text-stone-900 text-stone-700">
          {post.content.map((block, index) => {
            ...
            if (block.type === "image") {
              ...
              const src = blockId ? post.images[blockId as keyof typeof post.images] : (block as any).src;
            ...
            if (block.type === "callout") {
              ...
              const bgColor = isWarning ? "bg-red-50 border-red-200" : "bg-orange-50 border-orange-200";
              const titleColor = isWarning ? "text-red-900" : "text-orange-900";
              const Icon = isWarning ? <AlertCircle className="w-6 h-6 text-red-600 mr-4 mt-0.5 flex-shrink-0" /> : <Lightbulb className="w-6 h-6 text-orange-600 mr-4 mt-0.5 flex-shrink-0" />;

              return (
                <div key={index} className={`my-10 p-6 md:p-8 rounded-3xl border ${bgColor} flex items-start shadow-sm`}>
                  {Icon}
                  <div>
                    <h4 className={`font-bold uppercase tracking-widest text-sm mb-2 ${titleColor}`}>
                      {(block as any).title}
                    </h4>
